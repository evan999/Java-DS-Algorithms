...
class Result {
	 ...
	 */
	public static int queensAttack(int max, int num_of_obstacles, int q_row, int q_column,
		...
		// System.out.println("top:" + top + " left:" + left + " bottom:" + bottom + "
		// right:" + right);
		moves += (top + bottom + left + right);
		// BELOW FOR GETTING DIAGONAL MOVES
		// left up
		while ((++current_row > 0 && current_row <= max) && (--current_column > 0 && current_column <= max)) {
			if (obstacles.contains(new ArrayList<>(Arrays.asList(current_row, current_column))))
				break;
			moves++;
		}
...
public class Solution {
	public static void main(String[] args) throws IOException {
		...
		IntStream.range(0, k).forEach(i -> {
			try {
				obstacles.add(Stream.of(bufferedReader.readLine().replaceAll("\\s+$", "").split(" ")).map(Integer::parseInt)
						.collect(toList()));
